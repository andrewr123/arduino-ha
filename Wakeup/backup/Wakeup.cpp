 /*  
	*****************  WAKEUP  **********************
	...
	newly-vacant bunk, thus optimising access to free bunks
...
boolean WAKEUP::wakeMeAfter( void (*sleeper)(volatile void*), long ms, volatile void *context, boolean treatAsISR) {
...
void WAKEUP::timerISR() {							// Runs every heartbeat 
  ...
  for (int i = 0; i < _numSleepers; i++) {
	...
	// If countdown has finished then save values, sort out bunking and then handle sleeper
	if ((_bunks[i].timeToWake -= _heartbeat) <= 0) {  
		  ...
		  if (_bunks[i].treatAsISR == TREAT_AS_ISR) {
		  ...
		  // If one-shot sleeper then reduce count and test if any others left.  If no sleepers then stop heartbeat
		  // If sleepers left, then take sleeper in topmost bunk and move to the newly-vacant bunk (unless that was already the highest)
	      ...
	      else _bunks[i].timeToWake -=_bunks[i].sleepDuration;
        
    }
  }
  
  // Start the heartbeat for those sleepers left
  if (_numSleepers == 0) stopHeartbeat(); startHeartbeat();
  
  // Run the sleepers that were woken - be quick (and block runAnyPending() from being run)
  _inISR = true;
