...
#define MAXHEARTBEAT 8250						// in mS.  Round down from absolute max of 8,388,480 uS
...
public:
  ...
  boolean wakeMeAfter( void (*sleeper)(volatile void*), long ms, volatile void *context, boolean treatAsISR);	// Function to wake after expiry of ms.  If ms is negative then cycle repeats, else one-shot	
...
private:
  ...
  struct _bunk {								// 'Bunk' holding sleeper or empty
	void (*callback)(volatile void*);			// Callback function ('sleeper')
	boolean treatAsISR;							// True if callback to be immediate (as ISR).  False if to be put on pending queue
  	...
  	volatile void *context;								// For sleeper to interpret as appropriate when woken
